...
namespace Fuzzy_lib
{
	...
	*/
		...
		template <class Operation, typename std::enable_if<
			std::is_invocable_r<size_t, Operation, char_type*, size_t>::value, int>::type = 0>
		void resize_and_overwrite(size_t n, Operation op) {
			if (n > size()) {
				*this = basic_sstring(initialized_later(), n);
			}
...
namespace std
{
	...
	struct hash<Fuzzy_lib::basic_sstring<char_type, size_type, max_size, NulTerminate>> {
		size_t operator()(const Fuzzy_lib::basic_sstring<char_type, size_type, max_size, NulTerminate>& s) const {
